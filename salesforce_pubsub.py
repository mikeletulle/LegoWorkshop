...
def publish_robot_status(
    ...
    Additional fields can be passed via extra_fields (e.g. Case_Id__c).
    """
...
def _subscription_worker(cmd_queue: "queue.Queue[Dict[str, Any]]"):
    ...
    for fetch_response in response_stream:
        ...
        for event in fetch_response.events:
            ...
            # Updated to extract Case_Id__c and yield a dict
            command = record.get("Command__c")
            case_id = record.get("Case_Id__c")

            if command:
                _log.info("Received LEGO command: %s (Case: %s)", command, case_id)
                cmd_queue.put({"command": command, "case_id": case_id})
...
async def subscribe_to_commands() -> AsyncIterator[Dict[str, Any]]:
    """
    Async generator that yields a dictionary containing command info:
    {
      "command": "RECYCLING_OK",
      "case_id": "500xxxxx" (or None)
    }
    """
    logging.basicConfig(
        level=logging.INFO,
    ...
    while True:
        data = await asyncio.to_thread(cmd_queue.get)
        yield data
...
async def _test_read_commands():
    async for data in subscribe_to_commands():
        print(f"Command from Salesforce: {data['command']}, Case: {data.get('case_id')}")
